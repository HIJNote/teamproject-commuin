			...
			<div class="row">
				<div class="col-12 mt-4">
					...
					<form action="/board/update" method="POST">
						...
						<span class="custom-span" style="font-size: 22px">글쓰기 (수정)</span>
						</c:when>
						<c:otherwise>
						<span class="custom-span" style="font-size: 22px">채널공지쓰기 (수정)</span>
						</c:otherwise>
						</c:choose>
									...
									<c:if test="${empty check}">
										<c:forEach var="tag" items="${tag }">
											<c:if test="${tag.tg_name ne '공지' }">
												<c:choose>
													<c:when test="${tag.tg_name eq boardInfo.b_tag}">
						...
						<div class="form-group row">
							<div class="col-12 input-group pt-2">
								...
								<input type="text" name="b_title" class="form-control rounded-0"
									id="" value="${boardInfo.b_title }" aria-label="게시글 제목">
							</div>
						</div>
						...
						<div class="row">
							<div class="col-12">
								<textarea class="summernote" name="b_content"
									style="display: none;">${boardInfo.b_content }</textarea>
							</div>
							<script>
						...
						<div class="form-group row mt-3">
							<div class="col-12 text-right">
								<button type="submit"
									class="btn btn-outline-secondary rounded-0 custom-btn">수정</button>
							</div>
						</div>
					</form>
